...
class RAGService:
    ...
    async def search(
        ...
        if self.tenant_id:
            ...
            filter_metadata["tenant_id"] = str(query.tenant_id)
        if query.filters:
        ...
        # Source allowlist (tenant knowledge scope): empty allowlist = all
        # sources allowed; otherwise only listed sources may be retrieved.
        if query.allowed_sources:
            results = [
                r for r in results
                if query.is_source_allowed(str(r.document.metadata.get("source", "")))
            ]
    ...
    async def retrieve_context(
        ...
        query = KnowledgeQuery(query_text=query_text)
