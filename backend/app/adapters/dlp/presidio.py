...
Presidio is imported lazily so that the application can boot without it.
If it is missing, constructing PIIService raises a clear error; callers
should fail closed (e.g. refuse to process messages) rather than silently
skipping PII protection.
"""
    ...
    def __init__(self, config: PIIConfig | None = None):
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "Install them or disable ENABLE_PRESIDIO. PII protection "
                "fails closed: refusing to process messages without it."
            ) from e
